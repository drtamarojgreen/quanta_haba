...
std::map<std::string, std::string> quanta_config;
...
std::string call_quanta_model(const std::string& task) {
    if (quanta_config.count("model.llama_cli_path") && quanta_config.count("engine.model_path")) {
        std::string llama_path = quanta_config["model.llama_cli_path"];
        std::string model_path = quanta_config["engine.model_path"];
        
        std::stringstream cmd;
        cmd << llama_path << " -m " << model_path << " -p \"" << task << "\" -n 128 --temp 0 --no-display-prompt --log-disable --simple-io --single-turn --no-show-timings 2>/dev/null";
